...
const getAllCustomers = async (req, res) => {
    try {
        ...
        const customers = await Customer.find(searchQuery)
            .populate(
                "retailerId",
                "fullName shopName mobileNumber email address status"
            )
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limitNum)
...
const updateEmiPaymentStatus = async (req, res) => {
    try {
        ...
        if (
            isNaN(month) ||
            month < 1 ||
            month > customer.emiDetails.numberOfMonths
        ) {
            return res.status(400).json({
                success: false,
        ...
        // Find the EMI month
        const emiMonth = customer.emiDetails.emiMonths.find(
            (m) => m.month === month
        );
        ...
        return res.status(200).json({
            success: true,
            message: `EMI month ${month} marked as ${
                paid ? "paid" : "pending"
            }`,
            data: {
    ...
    try {
        ...
        if (!customer.fcmToken) {
            ...
            return res.status(400).json({
                ...
                } device. Customer device is not registered.`,
                error: "NO_FCM_TOKEN",
                data: {
                    customerId: customer._id.toString(),
                    customerName: customer.fullName,
        ...
        try {
            ...
            const fcmResult = await sendLockNotification(
                customer.fcmToken,
                isLocked
            );
            ...
            if (fcmResult.success) {
                ...
                console.log(
                    "Callback: POST /api/customer/device/lock-response"
                );
            } else {
...
const getEmiStatisticsAdmin = async (req, res) => {
    try {
        ...
        const paymentPercentage =
            totalEmiAmount > 0
                ? Math.round((stats.totalPaid / totalEmiAmount) * 100 * 100) /
                  100
                : 0;
    ...
    try {
        ...
        if (!customer.fcmToken) {
            ...
            return res.status(400).json({
                success: false,
                message:
                    "Customer device is not registered. No FCM token available.",
                error: "NO_FCM_TOKEN",
            });
        }
        ...
        const totalPendingAmount = pendingEmis.reduce(
...
const deleteCustomer = async (req, res) => {
    ...
    try {
        ...
        console.log(
            `🗑️  Deleted ${transactionsDeleted.deletedCount} transaction(s)`
        );
        ...
        console.log(
            `🗑️  Deleted ${assignmentsDeleted.deletedCount} recovery assignment(s)`
        );
            ...
            data: {
                ...
                deletionSummary: {
                    ...
                    totalRecordsDeleted:
                        ...
                        assignmentsDeleted.deletedCount,
                },
