...
const createCustomer = async (req, res) => {
  try {
    ...
    const {
      ...
      isAadhaarOtpVerified, aadhaarVerificationData
    } = req.body;

    // Parse aadhaarVerificationData if it's a JSON string
    let parsedAadhaarData = null;
    if (aadhaarVerificationData) {
      try {
        parsedAadhaarData = typeof aadhaarVerificationData === 'string'
          ? JSON.parse(aadhaarVerificationData)
          : aadhaarVerificationData;
      } catch (error) {
        return res.status(400).json({
          success: false,
          message: 'Invalid Aadhaar verification data format',
          error: 'INVALID_AADHAAR_DATA'
        });
      }
    }
    ...
    if (requiresAadhaarVerification) {
      // Check for either boolean flag or verification data
      const isVerified = (isAadhaarOtpVerified === 'true' || isAadhaarOtpVerified === true)
        || (parsedAadhaarData && parsedAadhaarData.verified);
      ...
      // If verification data is provided, validate it contains proper structure
      if (parsedAadhaarData) {
        if (parsedAadhaarData.aadhaarNumber && parsedAadhaarData.aadhaarNumber !== aadharNumber) {
          return res.status(400).json({
            success: false,
            message: 'Aadhaar number mismatch. Please verify with the same Aadhaar number used for registration.',
            error: 'AADHAAR_MISMATCH'
          });
        }

        // Ensure verification data contains reference ID (proof of successful verification)
        if (!parsedAadhaarData.referenceId) {
          return res.status(400).json({
            success: false,
            message: 'Invalid Aadhaar verification. Missing reference ID.',
            error: 'INVALID_AADHAAR_VERIFICATION'
          });
        }
      }
    }
